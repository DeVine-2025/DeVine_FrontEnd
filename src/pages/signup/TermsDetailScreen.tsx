  ...
  while (i < contentLines.length) {
    ...
    if (isTableLine) {
      ...
      if (rows.length > 0) {
        ...
        parsedContent.push(
          <div key={`table-${i}`} className="overflow-hidden rounded-xl border border-[var(--ui-100)]">
            <div className="grid" style={{ gridTemplateColumns: `repeat(${header.length}, minmax(0, 1fr))` }}>
              {header.map((cell, idx) => (
                <div
                  key={`th-${idx}`}
                  className="bg-[var(--ui-50)] px-3 py-2 text-[12px] font-semibold text-[var(--ui-800)]"
                >
                  {cell}
                </div>
              ))}
              {body.flatMap((row, rowIndex) =>
                row.map((cell, cellIndex) => (
                  <div
                    ...
                    className="border-t border-[var(--ui-100)] px-3 py-2 text-[12px] text-[var(--ui-700)]"
                  >
                    {cell}
                  </div>
                )),
...
const TermsDetailScreen = ({ open, title, content, onClose }: TermsDetailScreenProps) => {
  ...
  return (
    <div className="fixed inset-0 z-[60]" style={backgroundStyle}>
      ...
      <div className="mx-auto mt-[104px] flex w-full max-w-[632px] flex-col">
